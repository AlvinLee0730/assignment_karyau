          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(onPressed: () async {
                ...
                await supabase.from('guides').insert({'title': titleCtrl.text, 'category': uploadCat, 'content': contentCtrl.text, 'file_url': finalUrl, 'user_id': supabase.auth.currentUser!.id});
                _fetchGuides();
              ...
              itemBuilder: (context, i) {
                ...
                return Card(
                  ...
                  child: Column(children: [
                    ...
                    if (g.fileUrl != null)
                      GestureDetector(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (c) => GuideDetailPage(guide: g))),
                        child: Container(
                    ...
                    Padding(
                      ...
                      child: Row(children: [
                        ...
                        Text("${g.commentCount}"),
                        const Spacer(),
                        TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (c) => GuideDetailPage(guide: g))), child: const Text("View Details")),
                      ]),
                    )
                  ]),
                );
...
class _GuideDetailPageState extends State<GuideDetailPage> {
  ...
  Future<void> _fetchComments() async {
    ...
    setState(() { comments = res as List; loading = false; });
  }

  Future<void> _postComment() async {
    if (_commentCtrl.text.trim().isEmpty) return;
    await supabase.from('guide_comments').insert({'guide_id': widget.guide.id, 'user_id': supabase.auth.currentUser!.id, 'content': _commentCtrl.text.trim()});
    _commentCtrl.clear(); _fetchComments();
  }
